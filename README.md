...
座位顺序固定为 `自己 → 右家 → 对家 → 左家 → 自己`。实时状态机始终只提交当前应行动座位的出牌区域；不出仅由对应座位的 `passed.png` 模板判定。动作识别仍使用已选择的策略、牌型/压牌/剩余牌数/双副牌上限校验，并且将视觉未完成状态记录为自动重试，而不把不确定结果写入 reducer。两帧相同的明确花色结果即可确认；出现 `?` 时会额外观察一帧，优先等待特效消退后的完整花色，连续三帧仍遮挡才保留 `?` 继续流程。命中牌型特效后，当前突发样本会作废，动画消退后至少经过 450ms 静止窗口才会重新读牌。
