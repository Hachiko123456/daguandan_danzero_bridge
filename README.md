...
座位顺序固定为 `自己 → 右家 → 对家 → 左家 → 自己`。程序约以 10 FPS 连续采集，但不会按固定间隔直接相信某一帧：只监视当前预期玩家的出牌区域，高运动或特效命中会重新开始 300–500 ms 动态沉降，沉降后才读取 3–5 张稳定帧并进行多帧投票。默认单回合等待上限是 15 秒；超时只会要求人工确认。只有“出牌区持续为空”与“下一家的独立回合标记持续出现”同时成立时，才会提供一个仍需点击确认的“推断不出”候选。

自动候选还会校验牌型合法性、是否能压过桌面当前牌型、剩余张数和双副牌上限。自己出牌时会额外复用手牌区域识别，并要求连续三帧的出牌前后手牌多重集差值与出牌完全一致；校验失败不会推进状态。
