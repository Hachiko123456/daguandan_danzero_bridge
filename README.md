...
- 当前profile的session媒体默认总额度2GiB，可在其`profile.json`设置非负整数`recording_max_total_bytes`。达到限额停止新增录像、保留已录内容、标记PARTIAL并提示；识别和推荐继续，不自动删除旧录像。不同程序实例的录像预算不共享，勿将此视为多实例全局配额。
