...
座位顺序固定为 `自己 → 右家 → 对家 → 左家 → 自己`。实时状态机始终只观察当前应行动座位的出牌区域；不出仅由对应座位的 `passed.png` 模板判定。动作识别仍使用已选择的策略、牌型/压牌/剩余牌数/双副牌上限校验，并且将视觉未完成状态记录为自动重试，而不把不确定结果写入 reducer。命中牌型特效后，当前突发样本会作废，动画消退后至少经过 450ms 静止窗口才会重新读牌。

Reducer 一旦从已确认动作推导出轮到自己，就会异步计算 DanZero。建议完成后立即写进“对局动态”：橙色表示“待画面确认”，青绿色表示已经获得我方计时器或操作按钮旁证。尤其是“建议：不出”会在待确认阶段直接显示，不再因为快速过回合而丢失。旧状态版本的结果会记录为 `advice_stale`，不会覆盖当前建议。

“对局动态”是可鼠标选择和复制的统一时间线：每个动作恰好一行，出牌显示完整牌面，例如 `右家出牌：3♣ 3♠ 3?〔♠/♣〕 8♥ 8♣`，而不是 `2D` 一类内部码；不出、回合、接风、头游/二游/三游/末游、DanZero 建议和识别异常以不同颜色显示。`?` 只表示该张花色被遮挡，仍计入张数和牌型；后续清晰牌与它冲突时，程序只放宽这张历史未知花色的临时推断，并写入事件告警，明确识别的牌仍严格受双副牌上限约束。旧的“开始实时对局”和“最近动作快速纠错”控件已移除；历史 session、录像和日志不会因开始下一局而被清空。
