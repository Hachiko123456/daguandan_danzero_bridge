...
2. 监听页面不会创建 session、不会录制视频，也不会因为一帧误识别而开局；只有连续两次识别到完全相同的、已规范化的 27 张手牌，且级牌有效时，才进入实时状态机。默认开启“保存对局数据”，此时会同时创建 session 并开始录像；关闭该开关时，实时建议照常运行，但不写入 session、录像、时间线或训练样本。该开关会记住，并且只影响下一局。
