...
动作推进遵循固定座次：`自己 → 右家 → 对家 → 左家 → 自己`。当前玩家出牌/不出确认后，状态机只关心下一位应行动玩家的区域。`不出`不通过“区域为空”推断，而是只使用当前座位对应的 `passed.png` 模板。两帧相同且花色完整的结果即可确认；若任一张为 `rank?`，同一结果会再观察一帧，优先等待动画/按钮遮挡消退后的完整花色，连续三帧仍遮挡才提交 `?`，因此不会阻断流程。视觉未达到当前策略的稳定要求、牌型非法、不能压过桌面或违反剩余牌数/双副牌限制时，状态机写入“自动重试”而不污染 reducer。

## DanZero 建议显示

状态机已确认轮到自己时会立即异步请求 DanZero。建议具有两个显示级别：

- **橙色 DanZero 待确认**：模型已经完成，内容和推荐牌面（或“建议：不出”）立即可见；尚未取得我方计时器/按钮旁证。
- **青绿色 DanZero 建议**：同一请求已取得旁证，表示画面也支持当前轮到自己。

因此，模型完成并不需要等待下一帧旁证才展示；快速“不出”也不会吞掉已经算出的建议。`visible` 字段只表示展示级别，不表示建议是否存在。
...
| `recognition_retry` | 当前帧或当前策略尚不能提交动作 | 红色提示，不推进状态 |

`player_finished` 与 `wind_caught` 是只追加的辅助事件：它们用于时间线、日志和分析，不改变 reducer 的对局语义，也不改变 DanZero 输入。

第三名一经产生，四个名次已能确定，reducer 会清空当前墩并把 `current_player` 设为空。此后只轮询结算按钮，绝不伪造末游的“不出”、接风、回合开始或新的 DanZero 请求。若自己已经出完，当前应行动座位仍是唯一能提交正式动作的区域；同一帧可额外读取左家最近一条带 `?` 的出牌，但只有点数多重集和张数完全吻合且全部花色明确时才追加 `suit_corrected`，不会修改剩余牌数、当前回合、牌型、reducer 历史或 DanZero 输入。
...
在“标注与模板”页面把模板类型选为“牌型特效”即可采集：单张、对子、三张、三带二、钢板、三连对、连对、顺子、炸弹、同花顺、天王炸。请框选特效主体（不包含出牌牌面），每类保留 2–3 个无遮挡、不同动画时刻的样本。特效模板只用作读牌门控：命中会丢弃已经采集的突发帧，待特效消失后至少静止 450ms 再识别，不会据此直接判定牌型。
