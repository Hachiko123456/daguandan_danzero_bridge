...
4. “保存方式”默认是“对局录制”：在同一监听阶段分别确认27张初始手牌、合法级牌、首出与已发生的首手动作，再创建session；候选一致性不比较置信度或模板来源。“不保存”表示不生成对局 AVI，但仍创建耐久 session，保存小体积的正式规则事件、建议结果和错误日志，并在封局后自动导出诊断包。原“全程录制”改为“完整牌桌录制”，仅准备、发牌、对局时录制；大厅、未知页面、结算页只监听，不继续写视频。选项在开始监听后锁定。
...
极简浮窗默认 `500×245`、最小 `420×235`，顶部显示当前墩的 `自己｜右家｜对家｜左家` 出牌轨道，当前行动者高亮；下方只保留大号建议、牌型与牌，以及必要的一条错误。计算/确认期间使用简短稳定状态，不滚动显示内部“快速出牌补齐”“前手复读”等步骤。模型耗时、Q值、预选成功细节、日志路径和采集后端不占主建议区域，完整历史、回放、标注、模板与诊断仍在完整助手。两个窗口共享同一个 `LiveAssistantController`、V2 production runtime 和当前所选的 FableDan/DanZero worker。点击“打开完整助手”可恢复主窗口；关闭浮窗只隐藏浮窗并恢复主窗口，不会自行结束对局。

正常座位绑定“不出”优先直接交接，需新鲜标记、两个独立采集及合法轮次。历史缺失时不会伪造动作：己方机会的局部恢复预算为2秒，超时明确提示缺失座位/首出或动作，停止该旧窗口的重型重复扫描并继续轻量监听。高风险前手复核保留1.2秒截止，普通高可信动作不逐次阻挡推荐。跨圈、过期、重复帧不得被当成恢复前证据。

己方动作刚结束时残留的操作按钮不会被当作下一次己方回合。只有当前响应身份已经看见按钮明确消失，之后又由两张不同新采集确认按钮重新出现且活动座位为己方，才允许用这条边沿启动外部动作恢复；其他玩家仍可正常使用自己的行动倒计时。

同一动作窗口内，“初始可见的旧PASS”和“后来证实消失后重新出现的新PASS”分开处理，初始基线不再永久否决后续有效证据。短链可以包含多个连续非PASS，但每个座位都要有独立新画面、合法压牌关系和一致的采集时序；不会把后续回合的新牌填回旧回合。己方已认证交接保留最多1秒、4次的针对性读牌，仍验证已知手牌、牌型规则、新像素与双采集证据，不降低全局运动阈值。

“没有确认到合法牌”不等于“确认没有出牌”。非空低分结果、未分类的新像素、动画和识别异常都要保留为不确定，不能据此派生PASS；真实的新不出标记仍可独立确认实际动作。

名次图标只是生命周期证据，不能替代最后出牌。视觉上先出现二游/三游时，只要该座位在正式历史里仍有剩余牌，程序不会直接把它设为0。终局相邻动作只在同一采集代次和动作epoch内、两个不同capture得到完整一致链、各座位ROI为未消费新画面且按牌型规则依次合法时原子提交；随后名次事件引用最后动作。只有名次、重复帧、旧静态牌、跨代次或立即出现结算按钮都不能制造动作。

特殊皮肤的王牌仅在原整牌候选和颜色校验已成立时，使用对应同标签模板的完整纵向JOKER文字列进行局部复核，保留实际测量分数。红/黑颜色门防止大小王互换；不会从边框或任意文字生成新牌、不会降低全局准入阈值。用户ROI与模板配置不因这项复核而改变。

独立“要不起”规则提示不依赖完整模型历史，但要求同一局与采集代次、明确己方视觉证据、合法按钮框/得分、两张新鲜帧且无动画/遮挡/终局。它只显示“不出”，不提交PASS、不解锁坏历史模型；按钮消失、500ms证据过期、暂停或换源时撤销。当前建议与会话、采集代次、回合、状态版本和递增更新序号绑定；不能靠显示旧牌来掩盖等待。

性能诊断按真实处理时钟记录采集、队列、录像准入等待、分析、模型与GUI接收，并统计失败和无结果；`pipeline_timing`摘要随既有manifest导出，不新增大量媒体。每阶段近期128样本的百分位仅描述该窗口，不能充当整局P95。正式动作JSONL仍同步持久化，派生Markdown在访问或封局时流式生成，减少热路径写盘。
...
- `LiveV2SessionRuntime` 通过唯一 `RuleSession` 提交正式动作，并协调四座位观察、版本化建议机会及独立 Vision/Advice worker；旧 `LiveOrchestrator` 只保留在历史回放兼容路径，不参与生产实时对局。
...
动作推进遵循固定座次：`自己 → 右家 → 对家 → 左家 → 自己`。V2 观察器每帧轻量探测四个座位，并以有界队列轮转深读发生变化或饥饿的区域；当前行动者只影响优先级，不会让其他座位的新证据消失。`不出`不通过“区域为空”推断，而是只使用当前座位对应的 `passed.png` 模板。候选必须保留原采集代次、帧序号、动作 epoch 和证据引用，再由统一规则事务确认。
...
状态机确认轮到自己后，先保留默认 `200ms`、可配置范围 `150–250ms` 的本地“不出”确认窗口。窗口内两张新鲜帧确认 `cannot_beat` 按钮时，直接显示“不出”，不提交模型任务，也不写正式 PASS；窗口届满没有合法提示时，才异步请求 UI 当前选择的 FableDan 或 DanZero worker。时间线只显示面向用户的建议、合法本地“不出”或明确失败原因。

模型结果仍需匹配当前 session、采集代次、正式 revision/turn 和机会编号；只有真正被当前 session 接纳后才记为 ready。模型已经启动后才确认本地“不出”时，该机会由 local-pass 终结，迟到模型结果标记为 stale 并忽略。
...
“对局录制”和“完整牌桌录制”都会在完整对局开始后写入（达到录像容量后停止媒体写入、保留文本并标记partial；不会自动删除旧录像）：
