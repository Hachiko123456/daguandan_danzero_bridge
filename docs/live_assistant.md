...
动作推进遵循固定座次：`自己 → 右家 → 对家 → 左家 → 自己`。当前玩家出牌/不出确认后，状态机只关心下一位应行动玩家的区域。`不出`不通过“区域为空”推断，而是只使用当前座位对应的 `passed.png` 模板。视觉未达到当前策略的稳定要求、牌型非法、不能压过桌面或违反剩余牌数/双副牌限制时，状态机写入“自动重试”而不污染 reducer。
...
“对局动态”统一显示状态机事件和 DanZero 建议，并支持鼠标选择、复制和自动滚动到底部。每个动作占一行，所有牌都以点数和花色牌面显示，例如 `左家出牌：Q♣ Q♥ Q♠ Q♠`；日志不会把内部码（例如 `2D`）作为用户可见牌面。单张花色受遮挡时会显示为 `8?〔♠/♣〕`，而不是丢弃整组牌或把整手标成“花色待定”。
