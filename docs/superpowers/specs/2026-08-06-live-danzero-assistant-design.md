...
4. **两级稳定**：先判断画面是否可读，再判断语义结果是否足以提交。
...
- `stability.py`：ROI 运动评分、迟滞阈值、沉降窗口和特效阻断。
- `consensus.py`：按 `turn_id` 聚合多帧候选并输出可信度与冲突原因。
- `state_machine.py`：对局生命周期、视觉门和行动循环状态机。
...
状态中维护每家剩余牌数和已出完座位。某家确认出牌后剩余数为 0，则追加 `player_finished`。推导下一行动者时跳过已出完座位，但屏幕计时器仍是外部事实来源；推导值和计时器不一致时进入人工确认，而不是自动覆盖。

本墩维护最后一个非不出玩家。当其他仍在场玩家均确认不出，或计时器可靠地回到最后非不出玩家时，追加 `trick_started` 并清空当前墩动作。只有计时器换人而没有可靠的出牌或不出证据时，不自动推断“不出”，而是进入确认状态。
...
`INITIALIZING` 至少确认级牌、百搭级别、初始手牌、当前行动者和本墩首出者。初始化未完成时可以录像，但不能调用 DanZero。
...
```text
UNSTABLE ─连续低运动→ SETTLING ─满足沉降窗口→ STABLE
STABLE ─高运动/锚点丢失/effect 命中→ UNSTABLE
```

进入稳定使用较严格的低运动阈值，退出稳定使用较宽松的高运动阈值，形成迟滞，避免在边界上频繁抖动。初始建议值为：

- 采集目标：10 FPS，可配置范围 8–12 FPS；
- 沉降窗口：连续 300–600 ms 低运动；
- 语义共识：连续 3 个相同候选，或 500–800 ms 窗口中的合法多数候选；
- 识别目标：5 FPS，可配置范围 3–6 FPS。
...
```text
WAIT_TURN
  → OBSERVING_ACTION(player, turn_id)
  → CANDIDATE(action)
  → CONFIRMING
  ├─多帧一致、顺序合法、下一行动者稳定→ COMMIT
  ├─后续证据排除冲突→ COMMIT
  └─仍冲突、超时或非法→ REVIEW_REQUIRED
COMMIT → ADVANCE_TURN → WAIT_TURN
```

候选事件的指纹至少包含 `turn_id`、玩家、是否不出和规范化牌组。因此上一手牌仍停留在桌面时，不会在新 `turn_id` 中仅因画面相同而重复提交。

### 6.4 自己回合与 DanZero

当计时器连续稳定地指向自己，Reducer 能生成无 `readiness_errors` 的 `GuanDanState`，且当前 `turn_id` 尚未成功请求建议时，异步调用 DanZero。请求键为：

```text
(session_id, turn_id, state_revision)
```

同一个键只能产生一次有效建议。状态在建议返回前发生修正时，旧结果标记为 `advice_stale`，不显示为当前建议，并用新状态版本重新请求。
...
- 帧进入最近 10 秒的内存环形缓冲，同时分发给录像器和稳定门。
...
- 快速路径只处理锚点、运动、计时器、不出和 effect 阻断；稳定后才按当前玩家识别对应出牌区，并在需要时识别手牌。

### 7.2 稳定判定

运动评分只使用会影响状态的 ROI：手牌区、四家出牌区、首出/不出标记区和动作相关区域。持续变化的计时数字、头像和背景动画不参与像素运动评分；当前行动者使用 `timer/active` 模板判断。

每个 ROI 先灰度化和轻度模糊，再计算归一化绝对差分和变化像素比例。锚点缺失、关键 ROI 高运动、已知 effect 模板命中或窗口几何变化，都会使视觉门进入 `UNSTABLE`。effect 模板只作为“画面不可读”信号，不输出特效类别。

视觉稳定并不等于事件可信。特效可能短暂停顿，上一手牌也可能保持静止；因此稳定帧仍必须经过语义共识、计时器变化和牌局规则校验。
...
3. 重构截图为持久采集源，为识别服务加入模板缓存、快速视觉门和按需区域识别，同时保持单图接口兼容。
4. 实现实时 Orchestrator、共识状态机、事故触发和 DanZero 幂等调用。
