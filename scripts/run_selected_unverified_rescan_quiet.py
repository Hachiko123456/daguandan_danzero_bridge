...
if {x.session_id for x in desc} != IDS: raise SystemExit('session selection mismatch')
last={'percent':-1}
...
result=UnverifiedBatchScanService().scan(desc,profile_root=profile,output_root=out,max_workers=2,on_progress=progress)
print(json.dumps({'output_directory':str(result.output_directory),'summary_path':str(result.summary_path),'selected':result.selected_count,'completed':result.completed_count,'failed':result.failed_count,'cancelled':result.cancelled},ensure_ascii=False),flush=True)
