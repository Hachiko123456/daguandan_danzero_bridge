...
def default_return_card(cards, lv):
    cand = [c for c in cards if rank_of(c) <= 9 and not is_wildcard(c, lv)]
    if not cand:
...
class GuandanRound:
    ...
    def _do_tribute(self):
        ...
        n_bj = sum(1 for p in payers for c in self.hands[p] if rank_of(c) == BJ)
        if n_bj >= 2:
            self.resist = True
        ...
        if kind == "single":
            ...
            if o1 > o0:
                pay_pairs = [
                    (payers[1], t1, first),
                    (payers[0], t0, partner(first)),
                ]
                self.lead_player = payers[1]
            else:
                pay_pairs = [
                    (payers[0], t0, first),
                    (payers[1], t1, partner(first)),
                ]
...
def play_round(agents, rng=None, level=None, tribute_mode="random", sample_cb=None):
